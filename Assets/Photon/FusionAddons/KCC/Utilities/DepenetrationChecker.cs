	...
	public sealed class DepenetrationChecker : MonoBehaviour
	{
		...
		private Vector3 DepenetrateSingle(KCCOverlapInfo overlapInfo, KCCData data, Vector3 basePosition, Vector3 targetPosition, bool probeGrounding)
		{
			...
			if (probeGrounding == true && data.IsGrounded == false)
			{
				...
				bool isGrounded = KCCPhysicsUtility.CheckGround(_collider, targetPosition, hit.Collider, hit.Transform.position, hit.Transform.rotation, _radius, _height, _extent, minGroundDot, out Vector3 checkGroundNormal, out float checkGroundDistance, out bool isWithinExtent);
				if (isGrounded == true)
				{
